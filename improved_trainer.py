...
class ImprovedLocalModule(pl.LightningModule):
    def __init__(self, backbone, loss_networks, metrics_fn, cus_transforms, args, num_classes, valid_as_train=True):
        super(ImprovedLocalModule, self).__init__()
        # disable auto optim
        ...
        if len(args.delay_epochs) == self.K:
            self.delay_epochs = args.delay_epochs
        else:
            assert len(args.delay_epochs) == 1
            self.delay_epochs = [i * args.delay_epochs[0] for i in range(self.K)]
        self.delay_epochs.insert(0, 0)  # so that K=1, 2, ... can be index (the same as loss_nets)
        ...
        while retry_time < 2:
            try:
                ...
                for cur_k in range(1, self.K + 1):
                    if self.current_epoch < self.delay_epochs[cur_k]:
                        break
    ...
    def configure_optimizers(self):
        ...
        if self.args.decay_multi_epochs is None:
            cus_sch = lr_scheduler.StepLR(cus_optimizer, self.args.decay_epochs, gamma=self.args.decay_rate,
