...
class InfoProDecoderImg(BaseClassifier):

    def __init__(self, inplanes, interpolate_mode='bilinear', middle_planes=32, outplanes=64, loss=None):
        super(InfoProDecoderImg, self).__init__(require_x=True, return_y=False, require_label=False, return_loss=True)
        ...
        # self.loss = nn.MSELoss() #nn.BCELoss()
        self.loss = nn.BCELoss() if loss is None else loss
    ...
    def _image_restore(self, normalized_image):
        return normalized_image.mul(self.std) \
               + self.mean
    ...
    def forward(self,  features, x=None, label=None):
        ...
        image_rec = self.decoder(features_out)
        # print(image_rec.max(), image_rec.min(), image_ori.max(), image_ori.min())
        return self.loss(image_rec, self._image_restore(image_ori))
...
class RandomInfoProDecoderImg(InfoProDecoderImg):
    def __init__(self, inplanes, up_scale, patch_size, num_patches, interpolate_mode='bilinear', middle_planes=32, outplanes=64):
        super(RandomInfoProDecoderImg, self).__init__(inplanes, interpolate_mode, middle_planes, outplanes)
    ...
    def forward(self, features, x=None, label=None):
        features_large, image_ori_large = features, x
        image_ori_large = self._image_restore(image_ori_large)
        sampling_space = features_large.shape[-2] - self.patch_size, features_large.shape[-1] - self.patch_size
